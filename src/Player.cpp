...
void Player::forage()
{
    int ingredientIndex = std::rand() % masterList.getSize();
    int quantity = (std::rand() % 4) + 1; // Random quantity between 1 and 4
    Ingredient *ingredient = ingredientList.getByIndex(ingredientIndex);
    if (ingredient != nullptr)
    {
        InventoryItem foragedItem(*ingredient, quantity);
        inventory.addItem(foragedItem);
        std::cout << "You foraged: " << ingredient->getName() << " x " << quantity << std::endl;
    }
    else
    {
        std::cerr << "Error: Could not forage an ingredient." << std::endl;
    }
}
    // potions:
    ...
    void Player::updatePlayerIngredientList(const std::string &ingredientName, const std::array<Effect, 4> &masterEffects)
    {
        ...
        if (playerIngredient != nullptr)
        {
            ...
            std::cerr << "you know all effects for ingredient: " << ingredientName << std::endl;
        }
        else
        {
...
void Player::decreaseInventoryItemQuantity(const std::string &name){
    inventory.decreaseItemQuantity(name,1);
}